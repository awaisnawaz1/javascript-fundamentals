...
function basicMathExample(num1, num2)  {
  ...
  console.log(`Answer for Addition ${total}, Subtraction ${total2},
...
/* Exponentaiation is the rasing to the power of */
...
/* Comparison Operators - Which provice a boolean
respose of ture or false for each operator. */
...
const lesserEqual = (seventh, eight) => seventh <= eight;
...
function logicalExample(third, fourth) {
    const isvalid = (third < 10 && fourth > 4) && (third === 7 || fourth == 8);
    console.log(`with more operators becomes more responsibility ${isvalid}`);

}

logicalExample(6,8);

/* Increment - Value is incresed by 1 pre - increments
first before computation post - computation goes first,
then increments */ 
function incrementExample() {
    let able = 33;
    const bravo = able++
    console.log(`PostIncrement - able ${able},  bravo ${bravo}`);
}
...
function incrementPre()  {
    let hotel = 45;
    const jay = ++hotel;
    console.log(` Preincrement: hotel ${hotel}, jay ${jay}`);

}

incrementExample();

/* Decrement - Value is decrease by 1 pre - decrement first before 
computation post - computation goes first, then decrements*/
